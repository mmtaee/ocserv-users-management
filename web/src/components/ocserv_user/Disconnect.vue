...
<template>
  ...
  >
    <template #dialogTitle>
      <v-icon class="mb-1" start>mdi-lan-disconnect</v-icon>
      <span class="text-capitalize">{{ t("DISCONNECT_OCSERV_USER_TITLE") }} ({{ user.username }})</span>
    </template>

    <template #dialogText>
      {{ t("DISCONNECT_OCSERV_USER_MESSAGE") }} <br/><br/>
    </template>
