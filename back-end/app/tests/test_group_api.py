...
class OcservGroupApiTest(OcservTestAbstract):
    ...
    def setUp(self, *args, **kwargs) -> None:
        ...
        obj1, _ = OcservGroup.objects.get_or_create(name="test_group_delete1", desc="test group")
        self.group_list_id.append(obj1.id)

        obj2, _ = OcservGroup.objects.get_or_create(name="test_group_delete2", desc="test group")
        self.group_list_id.append(obj2.id)

        obj3, _ = OcservGroup.objects.get_or_create(name="test_group_delete3", desc="test group")
        self.group_list_id.append(obj3.id)

        obj4, _ = OcservGroup.objects.get_or_create(name="test_group_delete4", desc="test group")
    ...
    def group_update(self, group_id=None, staff=False, new_name=None):
        ...
        response = OcservGroupsViewSet.as_view({"patch": "partial_update"})(request, pk=_group_id)
        if group_id:
    ...
    def test_group_update(self, *args, **kwargs):
        ...
        with self.assertRaises(IndexError):
            self.group_update(group_id=self.group_list_id[100], new_name="changed_name_admin")
            self.group_update(
                group_id=self.group_list_id[100], staff=True, new_name="changed_name_staff"
            )

    def group_delete(self, group_id=None, staff=False):
        _group_id = group_id if group_id else 100  # override id with non exists id
        request = self.factory.delete(f"/groups/{group_id}/", headers=self.get_header(staff=staff))
        response = OcservGroupsViewSet.as_view({"delete": "destroy"})(request, pk=group_id)
        if group_id:
